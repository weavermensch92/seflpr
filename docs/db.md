...
| 자동 변환 | `postgresql://` → `postgresql+asyncpg://` (Railway 호환) |
...
- **개발**: 프로세스 수명 동안 동일한 랜덤 키 사용 (재시작 시 데이터 손실)
- **절대 코드에 하드코딩 금지**
...
- **도구**: Alembic
- **서버 시작 시**: `Base.metadata.create_all` 자동 실행 (테이블 없으면 생성)
